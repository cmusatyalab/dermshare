#!/usr/bin/env python
#
# Gemini -- Create Diamond filter and predicate
#
...
with open('fil_gemini', 'w+b') as f:
    ...
    for pyfile in sorted(glob.glob('gemini/*.py')):
...
manifest = parse_manifest(open('gemini.xml').read())
bundle_generic('gemini.pred', manifest, dict(fil_gemini='fil_gemini'))
